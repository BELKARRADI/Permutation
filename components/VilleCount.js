...
const VilleCount = () => {
  ...
  const generateTableData = () => {
    ...
    professeurs.forEach((prof) => {
      const villesDesirees = prof.villeDesiree.split(';');
      villesDesirees.forEach((ville) => {
        if (villeDesireeCounts[ville]) {
          villeDesireeCounts[ville]++;
        } else {
          villeDesireeCounts[ville] = 1;
        }
      });
    });

    const tableData = Object.entries(villeDesireeCounts).map(([villeDesiree, count]) => ({
      villeDesiree,
      count,
    }));
  ...
  return (
    <SafeAreaView style={{ flex: 1, marginLeft: 30, paddingTop: 50 }}>
      <Card>
        ...
        >
          <Text style={{ flex: 1, fontWeight: 'bold', marginLeft: 20, paddingTop: 20 }}>Ville Désirée</Text>
          <Text style={{ flex: 1, fontWeight: 'bold', paddingTop: 20 }}>Nombre</Text>
        </View>
        <FlatList data={generateTableData()} keyExtractor={(item) => item.villeDesiree} renderItem={renderTableItem} />
        <PieChart
          data={chartData}
          width={300}
          height={200}
